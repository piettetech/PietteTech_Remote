/*
 *  PietteTech_Remote: ClientDHTBMP
 *
 ...
 *			create DHT and BMP sensor classes and read sensor data
...
*/
/**************************************************************************/
#define DHTPIN        2         // Pin for internal DHT sensor.
...
*/
/**************************************************************************/
PietteTech_DHT_U _dht_u[2];          // Temperature & Humidity

/**************************************************************************/
/*!
        BMP Device Driver Initialization
*/
/**************************************************************************/
#define BMPTYPE       BMP085               // BMP085 BMP180
PietteTech_BMP bmp;
PietteTech_BMP_U _bmp_u[2];                // Pessure & Temperature
...
//IPAddress _serverIP{ 192, 168, 1,105};
IPAddress _serverIP{ 192, 168, 1, 138};
//IPAddress _serverIP{ 192, 168, 1, 146}; // macbook
#define REMOTE_DEFINE_INTERVAL 7	// how often do we re-send definition packet
int _remoteDefineInterval;		// counter for re-sending definition packet
...
void process_sensors()
{
    ...
    // Get temperature event and print its value.
    _bmp_u[0].getEvent(&event);
    _bmp_u[0].getSensor(&sensor);
    _bmp_u[0].printSensorEvent(&event, sensor.name);

    // Get humidity event and print its value.
    _bmp_u[1].getEvent(&event);
    _bmp_u[1].getSensor(&sensor);
    _bmp_u[1].printSensorEvent(&event, sensor.name);

    // reset client buffers
    stream1.client(PIETTETECH_REMOTE_DEFAULT_PORT);

    if (++_remoteDefineInterval == REMOTE_DEFINE_INTERVAL) {
	_bDefine = true;
	_remoteDefineInterval = 0;
    }

    // add sensor data to buffer
    stream1.addData(&_dht_u[0], _bDefine);
    stream1.addData(&_dht_u[1], _bDefine);
    stream1.addData(&_bmp_u[0], _bDefine);
    stream1.addData(&_bmp_u[1], _bDefine);
...
void setup_bmp()
{
	// Setup BMP objects for each BMP sensor
    bmp.begin();
    _bmp_u[0].begin(&bmp, BMPTYPE, ++sensorId, SENSOR_TYPE_PRESSURE, "BMP");
    _bmp_u[1].begin(&bmp, BMPTYPE, sensorId, SENSOR_TYPE_AMBIENT_TEMPERATURE, "BMP");
//    for (n = 0; n < 2; n++)
//        _sensor[_num_sensors++] = &_bmp_u[n];
}
...
void setup_dht()
{
    ...
    _dht_u[0].begin(&dht, DHTTYPE,  ++sensorId, SENSOR_TYPE_AMBIENT_TEMPERATURE, "DHT");
    _dht_u[1].begin(&dht, DHTTYPE,  sensorId, SENSOR_TYPE_RELATIVE_HUMIDITY, "DHT");
//    for (n = 0; n < 2; n++)
...
void setup()
{
    ...
    Serial.println("Remote DHT Sensor Example v0.1");
    ...
    // Setup BMP objects for each BMP sensor
    bmp.begin();
    _bmp_u[0].begin(&bmp, BMPTYPE, 2, SENSOR_TYPE_PRESSURE);
    _bmp_u[1].begin(&bmp, BMPTYPE, 2, SENSOR_TYPE_AMBIENT_TEMPERATURE);
    ...
    _bmp_u[0].getSensor(&sensor);
    _bmp_u[0].printSensorDetail(&sensor);
    _bmp_u[1].getSensor(&sensor);
    _bmp_u[1].printSensorDetail(&sensor);
    ...
    stream1.addData(&_bmp_u[0], true);
    stream1.addData(&_bmp_u[1], true);
    stream1.sendData();
